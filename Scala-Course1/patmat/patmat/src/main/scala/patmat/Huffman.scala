...
object Huffman {
  ...
  def weight(tree: CodeTree): Int = tree match {
    case Leaf(c, w) => w
    case Fork(left, right, lc, w) => w
  }
  
  def chars(tree: CodeTree): List[Char] = tree match {
    case Leaf(c, w) => List(c)
    case Fork(left, right, lc, w) => lc
  }
 ...
 /**
  ...
  def decode(tree: CodeTree,  bits: List[Bit]): List[Char] = {
    def helper(tree: CodeTree, cur: CodeTree, bits: List[Bit]): List[Char] = bits match {
      case Nil => 
          cur match {
            case Leaf(c, w) => List(c)
      ...
      case y :: ys =>
        cur match {
          case Leaf(c, w) => c :: helper(tree, tree, bits)
          case Fork(l, r, c, w) => 
            if(y == 0) helper(tree, l, ys)
            else helper(tree, r, ys)
        }
    }
...
//  }
    ...
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
     def helper(tree: CodeTree, c : Char, v : Int) : List[Bit] = tree match {
       case Leaf(c1, w) => 
         if(c1 == c) List(v)
         else Nil
       case Fork(l, r, cs, w) =>
         if(cs contains c) v :: helper(l, c, 0) ::: helper(r, c, 1)
         else Nil
     }
     text match {
       case Nil => Nil
       case y :: ys => helper(tree, y, -1).tail ::: encode(tree)(ys)
     }
   }
...
//    }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
     val ct = convert(tree)
     def helper(t: List[Char]): List[Bit] = t match {
       case Nil => Nil
       case x :: xs => codeBits(ct)(x) ::: helper(xs)
     }
     helper(text)
    }
