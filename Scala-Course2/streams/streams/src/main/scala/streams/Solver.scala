...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors filter ( block => ! (explored contains block._1) )
  ...
  def from(initial: Stream[(Block, List[Move])],
      ...
      if(initial.isEmpty) Stream.Empty
        ...
        val more = for{
          (block, ls) <- initial
          neighbors = neighborsWithHistory(block, ls)
          next <- newNeighborsOnly(neighbors, explored)
        } yield next
        //println(more.toList)
        initial.head #:: (initial.tail ++ from(more, ((more map (_._1)).toSet) ++ explored))
      }
